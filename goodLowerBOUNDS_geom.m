...
for r=1:1:9
    ...
    if sign(boundPrecSTE(r,j)' - boundBIGNys(r,j)')>=0 && sign(boundPrecSTE(r,j+1)' - boundBIGNys(r,j+1)')<=0
        nu(r) = .5 + (alpha-.5) * ((j-1)/9);
        break    
...
for r=1:1:9
    figure(r)
    semilogy(.55:.05:.95,etr1(r,:)','r')
    hold on
    semilogy(.55:.05:.95,boundPrecSTE(r,:)','o-r')
    hold on
    semilogy(.55:.05:.95,etrBig(r,:)','b')
    hold on
    semilogy(.55:.05:.95,boundBIGNys(r,:)','o-b')

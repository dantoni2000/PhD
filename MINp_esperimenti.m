...
for tMV = 10:10:190
    ...
    for s = 4:2:tMV-4
        ...
        ratio(:,s/2-1) = sum(log(1+diag(G(k+5+1:n,k+5+1:n)))) / sum(log(1+diag(G(k+1:n,k+1:n))));
    ...
    [minTr,argminTr(tMV/10)] = min(squareboundTr');
    BestSqrtTr(tMV/10) = 1/mi * minTr;
    RatioTr(tMV/10) = ratio(argminTr(tMV/10));

    [minFro,argminFro(tMV/10)] = min(squareboundFro');
    BestSqrtFro(tMV/10) = 1/mi * minFro;
    RatioFro(tMV/10) = ratio(argminFro(tMV/10));

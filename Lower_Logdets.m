...
for tMV = 10:10:90
    ...
    for s = 4:2:tMV-4
        ...
        c_star(:,s/2-1) = (1 + 2*(k+5)/(p-1)).* norm(G(k+5+1:n,k+5+1:n)) + (2*exp(1)^2*(k+5+p)/(p^2 - 1)) .* norm(G(k+5+1:n,k+5+1:n).^0.5 ,'fro')^2;
        c2(:,s/2-1) = 2* (1+12*sqrt(exp(1)^5/2)*(k/(p+1))^2)* norm(G(k+1:n,k+1:n))^2 + (12*exp(1)^4)*(k+p).^2/((p+1)^3*(p-3)) * norm(G(k+1:n,k+1:n).^0.5,'fro')^4;
    end
    
    BestLowerTr(tMV/10) = (1+G(1,1))^-1.*(min(1+ c_star')).^-1 .* norm((G(tMV+5+1:n,tMV+5+1:n)).^0.5,'fro')^2;

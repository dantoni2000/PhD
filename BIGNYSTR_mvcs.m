...
for t=1:T
    ...
    for l=20:10:60
        ...
        % Nystrom grande su A
        [UBig,LhatBig] = Nystrom(A,l);  llBig = LhatBig(l,l);
        PBig = (llBig+mi)^0.5*UBig*(LhatBig+mi*eye(l))^-0.5*UBig' + (eye(n) - UBig*UBig');
        %trBig(l/10) = log((llBig+mi)^(-l)*prod(diag(LhatBig+mi*eye(l,l))));
        trBig(l/10-1,t) = -2*trace(logm((llBig+mi)^-0.5*PBig));
...
figure(2)
plot(mvecs,abs(mtrBig-trA)./abs(trA),'r')
hold on
plot([20:10:60],[20:10:60].^-0.5,'o-r')
hold on
plot(mvecs,abs(mtr5-trA)./abs(trA),'m')
hold on
...
legend('Big Nystrom', 'Nystrom 5Hutch 2Lanczos', 'Nystrom 1Hutch 5Lanczos', 'Nystrom 2Hutch 5Lanczos', '1Hutch l-Lanczos', '2Hutch 0.5l-Lanczos')
xlabel('matvecs')
ylabel('error')
...
figure(3)
plot(mvecs,Max_trBig,'r')
hold on
RR = plot(mvecs,mtrBig,'o-r')
hold on
plot(mvecs,Min_trBig,'r')
hold on
...
legend([RR, GG, YY, CC, BB],'Big Nystrom', 'Nystrom 1Hutch 5Lanczos', 'Nystrom 2Hutch 5Lanczos', '1Hutch l-Lanczos', '2Hutch 0.5l-Lanczos')
xlabel('matvecs')
ylabel('traces')

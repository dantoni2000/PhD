...
for tMV = 10:10:90
    ...
    for s = 4:2:tMV-4
        ...
        % LOWERboundPrecSTE(:,s/2-1) = (1+ (1+alpha).^2 .*alpha .* min(alpha,((1+2*k/(p-1)).*alpha.^(k+1)+(2*exp(1)^2*(k+p)/(p^2-1).* alpha.^(k+1) .* (1-alpha.^(n-k))./(1-alpha)))) +2.*(1+alpha).*min(alpha,((1+2*k/(p-1)).*alpha.^(k+1)+(2*exp(1)^2*(k+p)/(p^2-1).* alpha.^(k+1) .* (1-alpha.^(n-k))./(1-alpha))))).*(1+alpha).^2;
        LOWERboundPrecSTE(:,s/2-1) = (1+  (1+ 1/4 * alpha) .* min(alpha,((1+2*k/(p-1)).*alpha.^(k+1)+(2*exp(1)^2*(k+p)/(p^2-1).* alpha.^(k+1) .* (1-alpha.^(n-k))./(1-alpha)))) ) .*(1+alpha).^2;
        % LOWERboundBIGNys(:,s/2-1) = ( (1+(1+alpha).* min(alpha,((1+2*(k+5)/(p-1)).*alpha.^(k+6)+(2*exp(1)^2*(k+5+p)/(p^2-1).* alpha.^(k+6) .* (1-alpha.^(n-k-5))./(1-alpha)))) ).*(1+alpha) ).^2; 
        LOWERboundBIGNys(:,s/2-1) = ( (1+ 1/2 .* min(alpha,((1+2*(k+5)/(p-1)).*alpha.^(k+6)+(2*exp(1)^2*(k+5+p)/(p^2-1).* alpha.^(k+6) .* (1-alpha.^(n-k-5))./(1-alpha)))) ).*(1+alpha) ).^2; 

    end
    
    BESTLOWERboundPrecSTE = (min(LOWERboundPrecSTE')).^-1 .*2 .* alpha.^(2*tMV+2).* (1-alpha.^(2*(n-tMV)))./(1-alpha.^2);
    BESTLOWERBIGNys = (min(LOWERboundBIGNys')).^-1 .* alpha.^(2*tMV+12) .* ((1-alpha.^(n-tMV-5))./(1-alpha)).^2;
    BESTLOWERNONys = 10/l ./(1+1/2.* alpha).^2 .* alpha.^2.* (1-alpha.^(2*n))./(1-alpha.^2);
    ...
    hold on
    semilogy(alpha, BESTLOWERBIGNys,'-*b')
    hold on
